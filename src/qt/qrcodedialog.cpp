...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("NAMO")
    updateDisplayUnit();
}
...
{
    QString ret = QString("NamoCoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non NAMO unit input in lnReqAmount, we generate the URI with NAMO as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(namocoinUnits::format(namocoinUnits::NAMO, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
